...
import { Account, CreateAccountData, UpdateAccountData, ApiError } from '@/types/account';
...
class AccountService {
  ...
  async getAccountsByStatus(activeStatus: string): Promise<Account[]> {
    try {
      ...
      console.error(`Error fetching accounts by status ${activeStatus}:`, error);
      throw error;
    }
  }

  async getAccountsByGroupId(groupId: string): Promise<Account[]> {
    try {
      ...
      console.error(`Error fetching accounts by group ${groupId}:`, error);
      throw error;
    }
  }
