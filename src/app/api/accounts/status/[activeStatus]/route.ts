...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/api/accounts/status/${activeStatus}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Failed to fetch accounts by status' }));
